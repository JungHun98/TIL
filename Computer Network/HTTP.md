...
- 캐시 시간 초과
  ...
  - 그런데 캐시에 유효시간이 지난 데이터가 서버에서 변경되지 않았음에도 해당 과정을 거쳐야 할까? 
## 검증 헤더와 조건부 요청 1
...
- 304 Not Modified + 헤더 메타 정보만 응답(HTTP body는 없다)
...
- 조건부 요청 헤더
  ...
  - 조건이 만족하면(데이터 수정) `200 OK`
  - 조건이 만족하지 않으면(데이터 수정 x) `304 Not Modified`
- If-Modified-Since
...
- If-Modified-Since, Last-Modified 단점
  ...
  - 날짜 기반의 로직 사용
  - 실제 컨텐츠는 변경되지 않았지만 데이터를 건드려 날짜가 변경되는 경우에 변경된 것으로 취급
  - 서버에서 별도의 캐시 로직을 관리하고 싶은 경우에 사용할 수 없음(스페이스나 주석처럼 크게 영향이 없는 변경에서 캐시를 유지하고 싶은 경우)
- ETag, If-Nond-Match
  - 캐시용 데이터에 임의의 고유한 버전 이름을 달아둠
  - 데이터가 변경되면 이 이름을 바꾸어서 변경함(Hash 값 다시 생성)
...
- Cache-Control: no-cache
  - 데이터는 캐시해도 되지만 **항상** 원(origin) 서버에서 데이터가 변경되었는지 검증하고 사용해야 함
...
## 프록시 캐시
- 원(origin) 서버 직접 접근하면 시간이 많이 걸릴 수 있음
- 프록시 캐시 서버
  - origin 서버로 접근하기 전에 접근하는 서버, 빠르게 데이터를 받을 수 있다.
  - 최초 접근은 원 서버를 거쳐야 함
