...
## 전송방식
- 단순 전송
  - Content-Length값을 알고 있을 때 사용함
  - 그 길이에 해당하는 Content를 전송
- 압축 전송
  - Content-Encoding를 추가로 헤더에 넣어주고 압축한 뒤 전송
- 분할 전송
  - Transfer-Encoding: chunked를 헤더에 추가하고 분할한 byte의 크기만큼 서버에서 만들어지면 그만큼을 나눠서 전송, 모두 전송하면 \r\n을 전송
...
- Referer: 이전 웹 페이지 주소
  ...
  - Referer를 사용해서 유입 결로 분석 가능(티스토리 유입경로)
  - 요청에서 사용
...
- Date: 메시지가 발생한 날짜와 시간
  - 응답에서 사용
## 특별한 정보 - 실제 애플리케이션에 영향을 주는 정보
- :star: Host: 요청한 호스트 정보(도메인)
  - 요청에서 사용
  - **필수**
